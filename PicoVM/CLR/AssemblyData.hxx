...
public:
    ...
    struct CLIMetaDataTables {
        ...
        std::vector<std::vector<uint8_t> > _StandAloneSig;
        ...
        std::vector<std::vector<uint8_t> > _ModuleRef;
        std::vector<std::vector<uint8_t> > _TypeSpec;
        std::vector<ImplMapRow> _ImplMap;
        std::vector<FieldRVARow> _FieldRVA;
        std::vector<AssemblyRow> _Assembly;
        uint32_t _AssemblyProcessor;
